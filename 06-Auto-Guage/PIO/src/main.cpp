...
void loop()
{
    ...
    int targetValue = buttonPressed ? 0 : 250; // Target value based on button state
    ...
    } else if (currentValue >= 140 && currentValue <= 250) {
    ...
    snprintf(tripText, sizeof(tripText), "%.0f ", totalDistanceKm);
    ...
    float rangeKm = 4 * 3.22; // 482 km for 100% battery
    char rangeText[10]; // Ensure this buffer is large enough for your number
    snprintf(rangeText, sizeof(rangeText), "%.1fV", rangeKm);
